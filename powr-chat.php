<?php
     ...
     */
    /*
    Plugin Name: POWr Chat
    Plugin URI: https://www.powr.io/plugins/chat
    Description: Easily provide live customer support with Facebook Messenger from your website.  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////Create Chat widget/////////////////////////////////
    class Powr_Chat extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_chat',
                             __( 'POWr Chat' ),
                             array( 'description' => __( 'Chat by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-chat' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Chat.
        </p>
        <p>
    ...
    function register_powr_chat() {
      register_widget( 'Powr_Chat' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_chat' );
    //Create short codes for adding plugins anywhere:
    function powr_chat_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-chat' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-chat' label='$label'></div>";
      }else{
      	"<div class='powr-chat'></div>";
      }
    }
    add_shortcode( 'powr-chat', 'powr_chat_shortcode' );
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_chat_menu' );
    function powr_chat_menu() {
      add_menu_page( 'POWr Chat', 'POWr Chat', 'manage_options', 'powr-chat-settings', 'powr_chat_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_chat_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/chat';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/chat';
      }
    ...
    register_activation_hook(__FILE__, 'powr_chat_plugin_activate');
      add_action('admin_init', 'powr_chat_plugin_redirect');

      function powr_chat_plugin_activate() {
      add_option('powr_chat_plugin_do_activation_redirect', true);
      }
      ...
      function powr_chat_plugin_redirect() {
      if (get_option('powr_chat_plugin_do_activation_redirect', false)) {
          delete_option('powr_chat_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-chat-settings&' );
          }
       }
      }
